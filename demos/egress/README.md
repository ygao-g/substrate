...
- **Egress policy** — the gateway denies by default, so the demo Actor needs an `EgressPolicy`
  ...
  succeed. `kubectl ate create egress-policy` creates one from a manifest (step 2b below) and
  `kubectl ate get egress-policy` reads it back; the e2e suites create theirs with
  `e2e.EnsureEgressPolicy`. An `all` rule reproduces the pre-policy behavior.
...
# 2b. Allow the Actor's egress; without a policy the gateway denies everything.
kubectl ate create egress-policy egress-demo -a ate-demo-egress -f - <<'EOF'
rules:
- all: {}
EOF
# The gateway caches a missing policy as deny for 10s (--egress-policy-cache-ttl), so a fetch
# tried before 2b keeps failing for up to that long after the policy appears.
