...
# no-policy note, the not-found error for a name that is not an actor, create
# from a manifest without metadata, table and YAML output, a
# `get -o yaml | create -f -` round-trip into a second actor, the client- and
# server-side rejections, and the actor and its policy being gone once it is
# deleted. No actor is resumed, so the egress gateway and router are not
...
log "get on an actor that does not exist"
run kate get egress-policy "${SRC}-missing" -a "${ATESPACE}"
check "get on a missing actor fails" test "${RC}" -ne 0
check "  ...and says not found" grep -q "not found" <<<"${ERR}"
...
run kate get egress-policy "${SRC}" -a "${ATESPACE}"
check "get on a deleted actor fails" test "${RC}" -ne 0
