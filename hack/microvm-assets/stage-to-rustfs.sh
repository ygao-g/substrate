...
# Deliberately ahead of the rustfs-bucket-init Job in
# manifests/ate-install/kind/rustfs.yaml, which still pins 2.17.0: the botocore
# vendored into 2.17 has no IPv6 endpoint validator and rejects a bracketed literal
# outright, which is the only endpoint this script can build when IP_FAMILY=ipv6.
# The Job addresses rustfs by DNS name, so it is unaffected and not worth forcing a
# fresh in-cluster pull on every family. Do not go below 2.31 here.
