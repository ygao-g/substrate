...
if [[ $# -gt 0 ]]; then
  case "$1" in
    -h|--help)
      ...
      echo "                     e.g. 64:ff9b::/96 (default: empty, no NAT64). Needed where the host"
      echo "                     has no IPv6 egress of its own; leave unset where it does. Setup,"
      echo "                     prerequisites and caveats: docs/dev/ipv6-local.md."
...
if [[ -n "${IPV6_DNS64_PREFIX}" ]]; then
  ...
  if [[ "${IPV6_DNS64_PREFIX}" != *::/96 ]]; then
    echo "error: IPV6_DNS64_PREFIX must be a /96 ending in '::' (got '${IPV6_DNS64_PREFIX}')" >&2
    echo "       Set IPV6_DNS_UPSTREAM to an address inside the prefix yourself." >&2
    exit 1
  fi
fi
...
EOF
if [[ -n "${IPV6_DNS64_PREFIX}" ]]; then
  # The agent hardcodes the IPv4 /24 it maps Pod addresses into (no flag for
  # it), so a node's Pod CIDR cannot be wider than /120. This /112 is what makes
  # kube-controller-manager hand out /120s; kind's IPv6 default /56 does not.
  cat <<EOF >> "${ROOT}/bin/kind-config.yaml"
