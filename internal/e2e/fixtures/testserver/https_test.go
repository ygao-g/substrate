...
// tunnel, so these tests pin them against a loopback listener, where no gateway
// is involved.
...
// serveOriginLocal starts the origin on loopback with the minted credentials
// and returns its address. A real listener, so the handshake is the one the pod
// performs.
...
// The origin has to serve the leaf it was handed, under the name that leaf
// carries. A caller that trusts the minting CA reaches it; the same caller on
// the system roots must not, which is what makes this an origin the test owns
// rather than one the public PKI vouches for.
...
// The leaf's name has to be checked, not just its chain: the e2e caller dials a
// ClusterIP and supplies the name out of band, so a mismatch has to fail.
...
// Without credentials the subcommand must fail its flag validation rather than
// start a listener that no caller can complete a handshake with.
