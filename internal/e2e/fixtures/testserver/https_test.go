...
func mintOriginFiles(t *testing.T) (certFile, keyFile string, caPEM []byte) {
	...
	if err != nil {
		t.Fatalf("encoding the origin CA: %v", err)
	}
...
// its address, so the handshake under test is the one the pod performs. A
// serve failure (a bad credential pair, for instance) fails the test at
// cleanup instead of leaving every request to time out.
func serveOriginLocal(t *testing.T, certFile, keyFile string) string {
	...
	go func() { served <- server.ServeTLS(listener, certFile, keyFile) }()
	t.Cleanup(func() {
		_ = server.Close()
...
// The origin serves exactly the minted leaf: a caller trusting the minting CA
// under the right name reaches it, and every other verification path fails
// with a certificate error -- the ownership property the egress test relies
// on.
func TestHTTPSOriginHandshake(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name:   "minted CA and right name",
			config: &tls.Config{RootCAs: roots, ServerName: testOriginDNSName},
		},
		{
			name:            "system roots",
			config:          &tls.Config{ServerName: testOriginDNSName},
			wantVerifyError: true,
		},
		{
			name:            "wrong server name",
			config:          &tls.Config{RootCAs: roots, ServerName: "other.invalid"},
			wantVerifyError: true,
		},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			status, err := getHealthz(t, address, tc.config)
			if tc.wantVerifyError {
				var verifyErr *tls.CertificateVerificationError
				if !errors.As(err, &verifyErr) {
					t.Errorf("GET /healthz error = %v, want a certificate verification error", err)
				}
				return
			}
			if err != nil {
...
// caller can handshake with. Driving the root command also pins the https
// subcommand's registration.
func TestHTTPSCmdRequiresCredentials(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "no flags", args: []string{"https"}},
		{name: "cert only", args: []string{"https", "--cert=/nonexistent"}},
		{name: "key only", args: []string{"https", "--key=/nonexistent"}},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			...
			if err == nil || !strings.Contains(err.Error(), "required flag") {
				t.Errorf("testserver %v error = %v, want a required-flag error", tc.args, err)
			}
		})
	}
}
