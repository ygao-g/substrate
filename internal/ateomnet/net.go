...
func InstallActorNftablesRules(egressPort uint16) error {
	...
	// families; nat there needs Linux 5.2. That is not free on a dual-stack
	// pod: an inet nat chain registers the nat hooks for both families, so
	// IPv6 in this netns becomes conntracked where the ip table left it
	// untracked.
	//
