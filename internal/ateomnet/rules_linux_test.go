...
// TestActorNftablesRuleExprs is the only test that fails when the nfproto guard
// is dropped from an IPv4 match: an IPv4-only datapath keeps working without
// it, so every behavioral test in the package stays green.
//
